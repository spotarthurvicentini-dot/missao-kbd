...
sheets.Sessoes = { rows: [
  ...
  [new Date('2026-08-02T11:00:00-03:00'), 's2', 'SPI001'],
  [new Date('2026-08-03T10:00:00-03:00'), 's3', 'SPI001'],
  [new Date('2026-06-20T10:00:00-03:00'), 's0', 'SPI002'],
] };
sheets['Progresso Quiz'] = { rows: [
  [new Date('2026-07-10T10:00:00-03:00'), 'q1', 'SPI001', 'pantene', 'PANTENE', 'bond-repair', 'Bond Repair — 20% do Espaço', 3, 4, 75, '2027-01-01T00:00:00-03:00'],
...
sheets.Videos = { rows: [
  ...
  [new Date('2026-08-05T10:00:00-03:00'), 'v2', 'SPI001', 'PANTENE', 'Título alterado', 'video', 'progress', 80, 100, 80, false, 'session', 'device', 'pantene', 'bond-repair'],
] };
...
assert.equal(report.totals.accesses, 3, 'eventos brutos continuam disponíveis como diagnóstico');
assert.equal(report.totals.promoterDays, 2, 'promotor-dias deve deduplicar setor + data');
assert.equal(report.totals.attentionPeopleCount, 2, 'badge deve deduplicar pessoas entre regras');
assert.equal(report.totals.videoAveragePercent, 4, 'vídeo total deve arredondar somente após a agregação bruta');
...
assert.equal(report.weeklyTrend.reduce((sum, row) => sum + row.accesses, 0), 3, 'a tendência preserva eventos brutos somente para diagnóstico');
assert.equal(report.weeklyTrend.reduce((sum, row) => sum + row.activePromoters, 0), 1, 'cada ponto semanal deve deduplicar o promotor dentro do bucket');
...
const rawAccuracyAlerts = context.buildManagementAttention_([{ accesses: 1, completedKbdCount: 5, completionRate: 56, correct: 69, questions: 99, accuracy: 70 }]);
assert.equal(rawAccuracyAlerts.find((row) => row.id === 'low-accuracy').count, 1, 'alerta deve comparar a razão bruta, não o percentual arredondado');
